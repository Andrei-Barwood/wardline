...
def test_tcp_echo_schema_strips_unknown_fields():
    msg = {"type": "echo", "request_id": "req-1", "payload": "hello", "extra": "ignored"}
...
def test_tcp_bool_is_not_accepted_as_number():
    msg = {"type": "beacon", "client_id": "client-1", "token": "secret", "seq": True}
...
def test_udp_seq_rejects_bool_and_string():
    msg = {"type": "beacon", "client_id": "client-1", "token": "secret", "seq": "123"}
    with pytest.raises(WardlineError) as exc:
        validate_udp(msg)
    assert exc.value.code == ErrorCode.invalid_message
...
def test_nested_object_rejected():
    msg = {"type": "echo", "request_id": "req-1", "payload": {"nested": "value"}}
    with pytest.raises(WardlineError) as exc:
        validate_tcp(msg)
    assert exc.value.code == ErrorCode.invalid_message
